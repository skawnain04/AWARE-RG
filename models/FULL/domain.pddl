(define (domain cascading-rg)
  (:requirements :strips :typing)

  (:types
    node
  )

  (:predicates

    ;; Initial compromised assets
    (has-compromised-customer-pc)
    (has-compromised-engineering-workstations)

    ;; Generic graph connection
    (has-connected ?at - node ?to - node)

    ;; Windows server access and vulnerabilities
    (has-access-to-windows-server ?node - node)
    (has-vulnerability-CVE-2019-0575 ?node - node)
    (has-vulnerability-CVE-2019-0584 ?node - node)
    (has-vulnerability-CVE-2018-0538 ?node - node)

    ;; Remote code execution
    (has-done-remote-code-execution ?node - node)

    ;; Firewall/VPN/network access
    (has-improper-vpn-firewall-configuration ?node - node)
    (has-vulnerability-CVE-2018-0296 ?node - node)
    (has-access-to-network ?node - node)

    ;; PLC-side vulnerability/service access
    (has-vulnerability-CVE-2017-9312 ?node - node)
    (has-exploit-improperly-implemented-fields-in-stack ?node - node)
    (has-access-to-plc-port-1132-tcp ?node - node)

    ;; PLC consequence
    (has-done-denial-of-service-in-PLC1 ?node - node)
    (plc-offline ?node - node)
    (has-Allen-Bradley-L30ERMS-safety-devices-v30-and-earlier ?node - node)

    ;; Fault propagation due to compromised PLC
    (has-fault-valve-blocked-close-due-to-compromised ?node - node)
    (has-fault-manual-isolation-valve-close-due-to-compromised ?node - node)
    (has-fault-pilot-low-supply-pressure-due-to-compromised ?node - node)
    (has-fault-pilot-extinction-due-to-compromised ?node - node)

    ;; Final hazard
    (has-fault-flare-flameout)
  )

  ;; Access to Windows server from compromised customer PC.
  (:action access-to-windows-server-from-customer-pc
    :parameters (?server - node)
    :precondition (and
      (has-compromised-customer-pc)
    )
    :effect (and
      (has-access-to-windows-server ?server)
    )
  )

  ;; Access to Windows server from compromised engineering workstations.
  (:action access-to-windows-server-from-engineering-workstations
    :parameters (?server - node)
    :precondition (and
      (has-compromised-engineering-workstations)
    )
    :effect (and
      (has-access-to-windows-server ?server)
    )
  )

  ;; Exploit CVE-2019-0575 on Windows server to obtain RCE.
  (:action exploits-vulnerability-CVE-2019-0575
    :parameters (?at - node)
    :precondition (and
      (has-access-to-windows-server ?at)
      (has-vulnerability-CVE-2019-0575 ?at)
    )
    :effect (and
      (has-done-remote-code-execution ?at)
    )
  )

  ;; Exploit CVE-2019-0584 on Windows server to obtain RCE.
  (:action exploits-vulnerability-CVE-2019-0584
    :parameters (?at - node)
    :precondition (and
      (has-access-to-windows-server ?at)
      (has-vulnerability-CVE-2019-0584 ?at)
    )
    :effect (and
      (has-done-remote-code-execution ?at)
    )
  )

  ;; Exploit CVE-2018-0538 on Windows server to obtain RCE.
  (:action exploits-vulnerability-CVE-2018-0538
    :parameters (?at - node)
    :precondition (and
      (has-access-to-windows-server ?at)
      (has-vulnerability-CVE-2018-0538 ?at)
    )
    :effect (and
      (has-done-remote-code-execution ?at)
    )
  )

  ;; Path 1: Remote code execution gives access to the network.
(:action gain-network-access-from-remote-code-execution
  :parameters (?at - node ?to - node)
  :precondition (and
    (has-done-remote-code-execution ?at)
    (has-connected ?at ?to)
  )
  :effect (and
    (has-access-to-network ?to)
  )
)

;; Path 2: CVE-2018-0296 / improper VPN-firewall configuration gives network access.
(:action exploits-vulnerability-CVE-2018-0296
  :parameters (?at - node ?to - node)
  :precondition (and
    (has-improper-vpn-firewall-configuration ?at)
    (has-vulnerability-CVE-2018-0296 ?at)
    (has-connected ?at ?to)
  )
  :effect (and
    (has-access-to-network ?to)
  )
)

  ;; Network access gives access to PLC port 1132/TCP.
  (:action access-to-plc-port-1132-tcp
    :parameters (?at - node ?plc - node)
    :precondition (and
      (has-access-to-network ?at)
      (has-connected ?at ?plc)
    )
    :effect (and
      (has-access-to-plc-port-1132-tcp ?plc)
    )
  )

  
(:action exploits-vulnerability-CVE-2017-9312
  :parameters (?at - node ?to - node)
  :precondition (and
    (has-connected ?at ?to)
    (has-access-to-network ?at)
    (has-access-to-plc-port-1132-tcp ?to)
    (has-vulnerability-CVE-2017-9312 ?to)
    (has-exploit-improperly-implemented-fields-in-stack ?to)
    (has-Allen-Bradley-L30ERMS-safety-devices-v30-and-earlier ?to)
  )
  :effect (and
    (has-done-denial-of-service-in-PLC1 ?to)
    (plc-offline ?to)
  )
)

  ;; Cyber-to-physical resiliency edge:
  ;; PLC DoS/offline causes valve blocked close.
  (:action causes-fault-valve-blocked-close
    :parameters (?plc - node)
    :precondition (and
      (has-done-denial-of-service-in-PLC1 ?plc)
      (plc-offline ?plc)
    )
    :effect (and
      (has-fault-valve-blocked-close-due-to-compromised ?plc)
    )
  )

  ;; Valve blocked close causes manual isolation valve close.
  (:action causes-fault-manual-isolation-valve-close
    :parameters (?plc - node)
    :precondition (and
      (has-fault-valve-blocked-close-due-to-compromised ?plc)
    )
    :effect (and
      (has-fault-manual-isolation-valve-close-due-to-compromised ?plc)
    )
  )

  ;; Manual isolation valve close causes pilot low supply pressure.
  (:action causes-fault-pilot-low-supply-pressure
    :parameters (?plc - node)
    :precondition (and
      (has-fault-manual-isolation-valve-close-due-to-compromised ?plc)
    )
    :effect (and
      (has-fault-pilot-low-supply-pressure-due-to-compromised ?plc)
    )
  )

  ;; Pilot low supply pressure causes pilot extinction.
  (:action causes-fault-pilot-extinction
    :parameters (?plc - node)
    :precondition (and
      (has-fault-pilot-low-supply-pressure-due-to-compromised ?plc)
    )
    :effect (and
      (has-fault-pilot-extinction-due-to-compromised ?plc)
    )
  )

  ;; Pilot extinction causes flare flameout.
  (:action causes-fault-flare-flameout
    :parameters (?plc - node)
    :precondition (and
      (has-fault-pilot-extinction-due-to-compromised ?plc)
    )
    :effect (and
      (has-fault-flare-flameout)
    )
  )
)